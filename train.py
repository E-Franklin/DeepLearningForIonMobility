...
def train_model():
    ...
    val_loader = load_test_data(collate_fn, scaler)
    ...
    for epoch in range(config.num_epochs):
        ...
        med_error = evaluate_model(model, config.model_name, train_loader,
                                   config, 'training', scaler)
...
def build_model(config):
    ...
    if config.model_type == 'LSTM':
        ...
        model = SequenceTransformer(get_vocab(), config.embedding_dim,
                                    config.num_attn_heads, config.dim_feed_fwd,
                                    config.num_layers, config.use_charge,

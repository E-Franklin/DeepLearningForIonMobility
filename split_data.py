...
def split_data(path, subsample=False, sample_size=None):
    ...
    train.to_csv(output + '_sample_train.tsv', sep='\t', index=False)
    test.to_csv(output + '_sample_test.tsv', sep='\t', index=False)
    val.to_csv(output + '_sample_val.tsv', sep='\t', index=False)
...
if len(sys.argv) == 2:
    ...
    print('Usage: split_data.py <data_path> <sub_sample: boolean> <sample_size: int>')
